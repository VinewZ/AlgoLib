...
export default function recursivelyPrintNumberOfShortestPaths(rows: number,cols: number):number{

  if(rows == 1 || cols == 1) return 1
  return recursivelyPrintNumberOfShortestPaths(rows - 1, cols) + recursivelyPrintNumberOfShortestPaths(rows, cols - 1)
}

const result = recursivelyPrintNumberOfShortestPaths(3, 7)
console.log(result)